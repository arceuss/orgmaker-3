...
//	MessageBox(hWnd,dropfile,"Drap",MB_OK);
	...
	//NoteWidth = 16; //Frame designationÅö
...
//	strcpy(name, MessageString[IDS_DEFAULT_ORG_FILENAME]);
	...
	case WM_COMMAND:
		...
		if(timer_sw==0){ //no while playing
			...
			case IDM_CT_L18:
			case IDM_CT_L19:
				SetUndo(); VolumeDecayEdit(1, -4, LOWORD(wParam) - IDM_CT_L1 + 1); //If the first argument is 0, it will be returned empty, so
				break;
			case IDM_CT_S0: 
			case IDM_CT_S1: 
			case IDM_CT_S2: 
...
//		MessageBox(hWnd,music_file,"",MB_OK);
	...
	case WM_DESTROY://for post-processing
		EndDirectSound();
		org_data.ReleaseNote();
		DeleteWaveData100(); //Added 20140401 Normally, it seems to be called in the order of WM_CLOSE Å-> WM_DESTROY Å-> WM_QUIT.
		EndGDI();
		if(!hDlgPlayer)DestroyWindow(hDlgPlayer);
		if(!hDlgTrack)DestroyWindow(hDlgTrack);
		if(!hDlgEZCopy)DestroyWindow(hDlgEZCopy);
